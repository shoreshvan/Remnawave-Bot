...
async def websocket_endpoint(websocket: WebSocket):
    ...
    if not token:
        ...
        await websocket.close(
            code=1008,
            reason=get_texts().t('API_WS_UNAUTHORIZED_NO_TOKEN', 'Unauthorized: No token provided'),
        )
        return
    ...
    if not await verify_websocket_token(websocket, token):
        ...
        await websocket.close(
            code=1008,
            reason=get_texts().t('API_WS_UNAUTHORIZED_INVALID_TOKEN', 'Unauthorized: Invalid token'),
        )
        return
